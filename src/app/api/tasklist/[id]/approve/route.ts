...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Hanya PM, ADMIN, SUPER_ADMIN yang bisa approve
  if (!['PM', 'ADMIN', 'SUPER_ADMIN'].includes(session.role)) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }

  // ✅ Validasi jam kerja - PM/ADMIN tidak boleh approve di luar jam kerja
  ...
  if (!workingHoursValidation.isAllowed) {
    console.log(`⏰ [Working Hours] Task approve blocked - outside working hours`);
  ...
  console.log(`✅ [Working Hours] Task approve allowed - within working hours`);
  ...
  if (task.status !== 'MENUNGGU_REVIEW_PM') {
    return NextResponse.json(
      { error: `Cannot approve. Current status: ${task.status}` },
      { status: 422 }
    );
  }
  ...
  try {
    await prisma.$executeRawUnsafe(
      `INSERT INTO public.tasklist_log ("taskId", "userId", keterangan, status, action)
       VALUES ($1, $2, $3, $4, $5)`,
      id, session.id, keterangan || 'Disetujui via mobile', 'SELESAI', 'status_change'
    );
  } catch {
    // log non-fatal
  }
