...
export async function POST(
  ...
  try {
    ...
    const { baId, moduleId, keterangan, gambar, fileName } = body;

    if (!baId || !moduleId) {
      return NextResponse.json({ success: false, error: 'baId and moduleId are required' }, { status: 400 });
    }

    // Calculate iteration from BA's rfcCount (all notes in this RFC round share same iteration)
    const bacara = await prisma.bacara.findUnique({
      where: { id: parseInt(baId) },
      select: { rfcCount: true },
    });
    const iteration = (parseInt(bacara?.rfcCount || '0') || 0) + 1;
