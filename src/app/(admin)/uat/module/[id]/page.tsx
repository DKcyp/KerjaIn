...
export default function UATModuleDetailPage() {
  const params = useParams();
  const router = useRouter();
  const moduleId = Number(params.id);

  const [moduleData, setModuleData] = useState<UATModuleData | null>(null);
  ...
  // Load UAT module data
  useEffect(() => {
    const fetchModuleData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/uat/module/${moduleId}`);
  ...
  // Load attachments and approval notes for this module's approval
  useEffect(() => {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      // Step 1: Create or update UAT approval for this module
      ...
      // Step 3: Update all UAT items in this module
      ...
      for (const item of pendingItems) {
        await fetch(`/api/uat/${item.id}`, {
          ...
          body: JSON.stringify({
            status: itemStatus,
            deskripsi: feedbackComment || item.deskripsi,
          }),
        });
      }
  ...
  if (!moduleData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-gray-600 dark:text-gray-400">UAT module tidak tersedia.</p>
        <button
          onClick={() => router.push("/uat")}
          className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors"
        >
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span>•</span>
              <span>{moduleData.totalItems} UAT Items</span>
              <span>•</span>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
              UAT Items dalam Module ({moduleData.totalItems})
            </h2>
              ...
              {moduleData.items.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">
                    Belum ada UAT items untuk module ini
                  </p>
                  <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                    UAT items akan muncul otomatis ketika development tasks selesai
                  </p>
                </div>
              ) : (
                moduleData.items.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => router.push(`/uat/${item.id}`)}
                    className="flex items-start gap-4 p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer"
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">Total UAT Items</div>
              </div>
            ...
            {feedbackAction === "approve" ? (
              <>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    Approve all pending UAT items in this module
                  </p>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    Reject all pending UAT items in this module
                  </p>
                </div>
              </>
            )}
