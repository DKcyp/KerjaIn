...
export default function UserDropdown() {
  ...
  const { user, reload, ssoLogout } = useAuth();
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="block mr-1 font-medium text-theme-sm text-white">{displayName}</span>

        <svg
          className={`stroke-white/90 transition-transform duration-200 ${
            isOpen ? "rotate-180" : ""
          }`}
          width="18"
          height="20"
      ...
      >
        ...
        <button
          onClick={async () => {
            closeDropdown();
            try {
              // Use SSO logout which handles both local and SSO server logout
              await ssoLogout();
            } catch (error) {
              console.error('Logout failed:', error);
              // Fallback to manual redirect if SSO logout fails
