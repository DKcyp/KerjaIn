...
async function seedRBAC() {
  ...
  // Create Master Roles (mapped to existing Role enum)
  const roles = [
    {
      name: 'super_admin',
      displayName: 'Super Administrator',
      description: 'Full system access with all permissions (SUPER_ADMIN)'
    },
    {
      name: 'project_manager', 
      displayName: 'Project Manager',
      description: 'Manage projects, blueprints, and team assignments (PM)'
    },
    {
      name: 'developer',
      displayName: 'Developer/Programmer', 
      description: 'Development tasks, code management, and testing (PROGRAMMER)'
    },
    {
      name: 'admin',
      displayName: 'Administrator',
      description: 'Administrative work and system management (ADMIN)'
    },
    {
      name: 'tester',
      displayName: 'Tester',
      description: 'UAT and EUT testing, quality assurance (additional role)'
    }
  ];
  ...
  // Admin permissions (administrative work - similar to developer but with user management)
  const adminRole = await prisma.masterRole.findUnique({ where: { name: 'admin' } });
  const adminPermissions = [
    'user.create', 'user.read', 'user.update', 'user.delete',
    'project.read', 'blueprint.read', 'task.read', 'task.update',
    'uat.create', 'uat.read', 'uat.update', 'eut.create', 'eut.read', 'eut.update',
    'golive.read', 'report.view', 'report.export', 'system.settings', 'system.logs'
  ];
...
async function main() {
  ...
  await prisma.pegawai.create({
    data: {
      ...
      role: 'ADMIN',
      noUrut: nextNoUrut, // Dynamically calculate the next available noUrut
    },
  });
